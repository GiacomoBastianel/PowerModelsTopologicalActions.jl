...
for i in 1:5
    print("NEW BUS","\n")
    print("        ","\n")
    print("UE UAGLIO FACIMMO LU BUSBAR SPLITTING","\n")
    print("        ","\n")
...
obj = []
for i in 1:length(results["lpac"])
        push!(obj,results["lpac"]["$i"]["objective"])
end

results_dict_lpac = JSON.json(results)
folder_results = "/Users/giacomobastianel/Library/CloudStorage/OneDrive-KULeuven/Papers/Topological_actions/Results"

open(joinpath(folder_results,"case_67/Results_lpac_screening_1_71.json"),"w" ) do f
...
# -> this works!
splitted_bus_ac = 37
#-> it does not perform well for DC substations
splitted_bus_dc = 2
...
open(joinpath(folder_results,"case_67/Results_ac_bs_37_lpac.json"),"w" ) do f
    write(f,results_bs_lpac)
end
results_bs_ac = JSON.json(result_switches_AC_ac_ref)
open(joinpath(folder_results,"case_67/Results_ac_bs_37_ac.json"),"w" ) do f
...
prepare_DC_feasibility_check(result_switches_DC_ac_ref,data,try_dc_check,switches_couples_dc,extremes_ZILs_dc)
prepare_DC_feasibility_check(result_switches_DC_lpac_ref,data,try_dc_check_lpac,switches_couples_dc,extremes_ZILs_dc)



data_bs_lpac_check = JSON.json(try_ac_check_lpac)
open(joinpath(folder_results,"case_67/Data_ac_bs_38_lpac_check.json"),"w" ) do f
    write(f,data_bs_lpac_check)
end
data_bs_ac_check = JSON.json(try_ac_check)
open(joinpath(folder_results,"case_67/Results_ac_bs_38_ac_check.json"),"w" ) do f
...
open(joinpath(folder_results,"case_67/Results_ac_bs_38_lpac_check.json"),"w" ) do f
