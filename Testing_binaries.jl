...
result_AC_ots_5_lin = _PMTP.run_acdcots_AC_lin(data_5_acdc,ACPPowerModel,juniper; setting = s)

#result_AC_ots_5_dc    = _PMTP.run_acdcots_AC(data_5_acdc,DCPPowerModel,juniper; setting = s)
...
count_ = 0
for (br_id,br) in result_AC_ots_5_lin["solution"]["branch"]
    if br["br_status"] <= 0.5
        print(br_id,"\n")
        count_ += 1
    end
...
#for (br_id,br) in result_AC_ots_5_dc["solution"]["branch"]
#    print(br["br_status"],"\n")
#end

for (b, branch) in result_AC_ots_5_lin["solution"]["branch"]
    if branch["br_status"] < 0.5
        data_5_acdc["branch"][b]["br_status_initial"] = 0
    else
        data_5_acdc["branch"][b]["br_status_initial"] = 1
    end
end

result_AC_ots_5_linearised = _PMTP.run_acdcots_AC_linearised(data_5_acdc,ACPPowerModel,ipopt; setting = s)

count_ = 0
for (br_id,br) in result_AC_ots_5_linearised["solution"]["branch"]
    #print(br["br_status"],"\n")
    if br["br_status"] <= 0.5
        print(br_id,"\n")
        count_ += 1
...
for (b, branch) in result_AC_ots_5_linearised["solution"]["branch"]
    if branch["br_status"] < 0.5
