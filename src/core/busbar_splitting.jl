...
function prepare_AC_feasibility_check(result_dict, input_dict, input_ac_check, switch_couples, extremes_dict,input_base)    
    orig_buses = maximum(parse.(Int, keys(input_base["bus"]))) + length(extremes_ZILs_ac)
    ...
    for (sw_id,sw) in input_dict["switch"]
        if haskey(sw,"auxiliary") && haskey(sw,"auxiliary") # Make sure ZILs are not included 
            ...
            for zil in eachindex(extremes_ZILs_ac)
                if sw["bus_split"] == extremes_ZILs_ac[zil][1] && result_dict["solution"]["switch"]["$(switch_couples[sw_id]["switch_split"])"]["status"] == 1.0  # Making sure to reconnect everything to the original if the ZIL is connected
                    if result_dict["solution"]["switch"][sw_id]["status"] >= 0.9
                        if aux == "gen"
                            input_ac_check["gen"]["$(orig)"]["gen_bus"] = deepcopy(extremes_ZILs_ac[zil][1])
                        elseif aux == "load"
                            input_ac_check["load"]["$(orig)"]["load_bus"] = deepcopy(extremes_ZILs_ac[zil][1])
                        elseif aux == "convdc"
                            input_ac_check["convdc"]["$(orig)"]["busac_i"] = deepcopy(extremes_ZILs_ac[zil][1])
                        elseif aux == "branch"  
                ...
                elseif sw["bus_split"] == extremes_ZILs_ac[zil][1] && result_dict["solution"]["switch"]["$(switch_couples[sw_id]["switch_split"])"]["status"] == 0.0 # Reconnect everything to the split busbar
                    if result_dict["solution"]["switch"][sw_id]["status"] >= 0.9
                        if aux == "gen"
