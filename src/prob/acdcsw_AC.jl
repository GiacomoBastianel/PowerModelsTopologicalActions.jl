...
export run_acdcsw_AC_lin
export run_acdcsw_AC_lin_constrained
export run_acdcsw_opf_AC
...
function run_acdcsw_AC_lin(file, model_constructor, optimizer; kwargs...)
    return _PM.solve_model(file, model_constructor, optimizer, build_acdcsw_AC_lin; ref_extensions=[_PMACDC.add_ref_dcgrid!,_PM.ref_add_on_off_va_bounds!], kwargs...)
end

""
function build_acdcsw_AC_lin(pm::_PM.AbstractPowerModel)
    # AC grid
    _PM.variable_bus_voltage(pm)
    _PM.variable_gen_power(pm)
    _PM.variable_branch_power(pm)
    ...
    for i in _PM.ids(pm, :switch)
        ...
        constraint_switch_voltage_on_off(pm,i) # making sure that the voltage magnitude and angles are equal at the two extremes of a closed switch
        constraint_switch_power_on_off(pm,i) # limiting the maximum active and reactive power through an ac switch
        constraint_linearised_binary_variable_switch_no_ZIL(pm,i)
    end
...
## AC formulation of the AC/DC OTS with only AC branches that can be switched, binary constraint for each branch linearised between 0 and 1 ##
function run_acdcsw_AC_lin_constrained(file, model_constructor, optimizer; kwargs...)
    return _PM.solve_model(file, model_constructor, optimizer, build_acdcsw_AC_lin_constrained; ref_extensions=[_PMACDC.add_ref_dcgrid!,_PM.ref_add_on_off_va_bounds!], kwargs...)
end

""
function build_acdcsw_AC_lin_constrained(pm::_PM.AbstractPowerModel)
    # AC grid
    _PM.variable_bus_voltage(pm)
    _PM.variable_gen_power(pm)
    _PM.variable_branch_power(pm)

    variable_switch_indicator_linearised(pm) # binary variable to indicate the status of an ac switch
    variable_switch_power(pm) # variable to indicate the power flowing through an ac switch (if closed)
    ...
    for i in _PM.ids(pm, :switch)
        ...
        constraint_switch_power_on_off(pm,i) # limiting the maximum active and reactive power through an ac switch
        #constraint_linearised_binary_variable_switch_no_ZIL(pm,i)
        constraint_linearised_binary_variable_switch(pm,i)
    end
...
function run_acdcsw_AC_lin_constrained_sp(file, model_constructor, optimizer; kwargs...)
    return _PM.solve_model(file, model_constructor, optimizer, build_acdcsw_AC_lin_constrained_sp; ref_extensions=[_PMACDC.add_ref_dcgrid!,_PM.ref_add_on_off_va_bounds!], kwargs...)
end

""
function build_acdcsw_AC_lin_constrained_sp(pm::_PM.AbstractPowerModel)
    # AC grid
    _PM.variable_bus_voltage(pm)
    _PM.variable_gen_power(pm)
    _PM.variable_branch_power(pm)

    variable_switch_indicator_linearised_sp(pm) # binary variable to indicate the status of an ac switch
    variable_switch_power(pm) # variable to indicate the power flowing through an ac switch (if closed)
...
function run_acdcsw_opf_AC(file, model_constructor, optimizer; kwargs...)
    return _PM.solve_model(file, model_constructor, optimizer, build_acdcsw_opf_AC; ref_extensions=[_PMACDC.add_ref_dcgrid!,_PM.ref_add_on_off_va_bounds!], kwargs...)
end

""
function build_acdcsw_opf_AC(pm::_PM.AbstractPowerModel)
    # AC grid
    _PM.variable_bus_voltage(pm)
    _PM.variable_gen_power(pm)
    _PM.variable_branch_power(pm)
    ...
    for i in _PM.ids(pm, :switch)
        constraint_switch_thermal_limit(pm, i) # limiting the apparent power flowing through an ac switch
        constraint_switch_voltage(pm,i) # making sure that the voltage magnitude and angles are equal at the two extremes of a closed switch
